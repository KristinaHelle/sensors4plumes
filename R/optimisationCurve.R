...
optimisationCurve = function(
  report, # report from optimiseSD, type depends on algorithm
...
#   }
         ...
         "global" = {
           ...
           detected = matrix(nrow = nrow(report$SD), ncol = ncol(report$SD))
           for (i in 1:nrow(report$SD)){
             for (j in 1:ncol(report$SD)){
               detected[i,j] = 
                 sum(report$report$detectable[report$SD[i,1:j],]) 
               if (j > 1){
