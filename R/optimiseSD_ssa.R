...
#     }
  ...
  out = list()
  out[["SD"]] = locationsOld
  out[["SD_best"]] = locationsBest
  out[["cost"]] = costOld
  out[["cost_best"]] = costBest
  out[["report"]] = report[1:k,]

  if (verbatim) {
    out[["SDs"]] = SDs[1:k,]
    out[["SDs_test"]] = SDs_test[1:k,]
    out[["SDs_best"]] = SDs_best[1:k,]    
    if (acceptanceMethod =="vanGroenigen") {
#      out[["costDiffEstimate"]] = costDiffEstimateAll
      out[["coolingPar"]] = c(cooling, startAcceptance)
    }
    out[["jumpBack"]] = jumpBack
  }
