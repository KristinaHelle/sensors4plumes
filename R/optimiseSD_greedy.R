...
determineNextStep = function(
  locationsCurrent,                      # indices of current sensors
  locationsFix,                      # indices of current sensors that are fix
  ...
  if (aimDifference == 0){
    ...
    if (length(setdiff(locationsCurrent, locationsFix)) == 0 & aimDifference > 0){
      NextStep = "stop"
      switch(kindAim,
